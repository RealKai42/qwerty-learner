...
const getWordDictationTypeList = (t: any): { name: string; type: WordDictationType }[] => [
  {
    name: t('switchers.dictation.types.hide_all'),
    type: 'hideAll',
  },
  {
    name: t('switchers.dictation.types.hide_vowel'),
    type: 'hideVowel',
  },
  {
    name: t('switchers.dictation.types.hide_consonant'),
    type: 'hideConsonant',
  },
  {
    name: t('switchers.dictation.types.random_hide'),
    type: 'randomHide',
  },
]
...
export default function WordDictationSwitcher() {
  ...
  const wordDictationTypeList = getWordDictationTypeList(t)
      ...
      {({ open }) => (
        <>
          <Popover.Button
            ...
            type="button"
            aria-label={t('switchers.dictation.title')}
          >
            ...
            <Popover.Panel className="absolute left-1/2 z-10 mt-2 flex max-w-max -translate-x-1/2 px-4 ">
              <div className="shadow-upper box-border flex w-60 select-none flex-col items-center justify-center gap-4 rounded-xl bg-white p-4 drop-shadow dark:bg-gray-800">
                <div className="flex w-full  flex-col  items-start gap-2 py-0">
                  ...
                  <div className="flex w-full flex-row items-center justify-between">
                    ...
                    <span className="text-right text-xs font-normal leading-tight text-gray-600">
                      {t('switchers.dictation.dictation_enabled', {
                        status: wordDictationConfig.isOpen ? t('settings.status.enabled') : t('settings.status.disabled'),
                      })}
                    </span>
                  </div>
                </div>
                ...
                >
                  <div className="flex w-full  flex-col  items-start gap-2 py-0">
                    <span className="text-sm font-normal leading-5 text-gray-900 dark:text-white dark:text-opacity-60">
                      {t('switchers.dictation.dictation_mode')}
                    </span>
                    <div className="flex w-full flex-row items-center justify-between">
